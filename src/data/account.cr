...
module TB::Data
  ...
  struct Account
    ...
    def self.read_next_final_contact
      TB::DATA.query_one?("SELECT accounts.* FROM accounts FULL OUTER JOIN balances ON id = account_id WHERE final_contact = false AND ((coin=2 or coin=4) and balance > 100) OR (coin=1 and balance>1) AND discord_id IS NOT NULL LIMIT 1", as: Account)
    end
    ...
    def balances
      TB::DATA.query_all("SELECT * FROM balances WHERE account_id = $1", @id, as: Balance)
    end
    ...
    def self.read(user_type : UserType, id : Int64) : self
      TB::DATA.query_one("INSERT INTO accounts(#{user_type}) VALUES ($1) ON CONFLICT (#{user_type}) DO UPDATE SET #{user_type} = accounts.#{user_type} RETURNING *", id, as: self)
    end

    def self.read(id : Int32) : self
      TB::DATA.query_one("SELECT * FROM accounts WHERE id = $1", id, as: self)
    end
