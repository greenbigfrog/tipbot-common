...
struct TB::Data::Coin
  ...
  def self.read
    TB::DATA.query_all("SELECT * FROM coins", as: self)
  end

  def self.read(id : Int32)
    TB::DATA.query_one("SELECT * FROM coins WHERE id = $1", id, as: self).not_nil!
  end

  def self.read_all
    hash = Hash(Int32, Coin).new
    TB::DATA.query_all("SELECT * FROM coins", as: self).each do |coin|
      hash[coin.id] = coin
    end
    hash
  end
  ...
  def self.read_discord_token(coin : Int32)
    TB::DATA.query_one("SELECT discord_token FROM coins WHERE id = $1", coin, as: String)
  end
end
